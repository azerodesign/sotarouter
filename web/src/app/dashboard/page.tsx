...
+      <main id="main-content" className="mx-auto max-w-[1360px] px-4 py-7 sm:px-6 lg:ml-64 lg:px-8 lg:py-10">
        ...
        {activeTab === "endpoint" && (
          <div className="rise-in space-y-7">
            <div><div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em] text-zinc-600"><span className="h-1.5 w-1.5 rounded-full bg-amber-400" /> Gateway access</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-white">Endpoint & key access.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-zinc-500">One request surface for your clients. The Go gateway prototype is ready locally; production connection remains intentionally disabled.</p></div>
            ...
            <section className="surface rounded-2xl p-6 sm:p-7"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-950 text-zinc-400"><Terminal className="h-4 w-4" /></span><div><p className="eyebrow">Request base</p><h2 className="mt-1 text-base font-semibold text-white">Ready for local gateway testing.</h2></div></div><div className="mt-5 flex flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-950 p-4 sm:flex-row sm:items-center sm:justify-between"><div><code className="font-mono text-xs text-emerald-300">http://localhost:8080/v1/chat/completions</code><p className="mt-2 text-xs text-zinc-600">OpenAI-compatible POST. Upstream forwarding is implemented in the Go prototype.</p></div><span className="shrink-0 rounded-md border border-amber-400/20 bg-amber-400/5 px-2.5 py-1.5 font-mono text-[10px] uppercase text-amber-300">SSE pending</span></div></section>
          </div>
        )}
        ...
        {activeTab === "providers" && (
          <div className="rise-in space-y-7">
            ...
            <section className="space-y-4"><div><p className="eyebrow">OAuth Providers</p><h2 className="mt-1 text-lg font-semibold text-white">Managed provider catalog</h2><p className="mt-1 text-sm text-zinc-600">Catalog only. Connection state appears after a manual import.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{["Claude", "Genspark", "Antigravity", "NARAROUTER"].map((name) => <article key={name} className="surface rounded-xl p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-950 font-mono text-xs font-semibold text-zinc-300">{name.slice(0, 2)}</span><div><h3 className="text-sm font-medium text-white">{name}</h3><p className="mt-1 font-mono text-[10px] uppercase text-zinc-600">OAuth catalog</p></div></div><span className="h-2 w-2 rounded-full bg-zinc-700" /></div><button type="button" onClick={() => setShowImport(true)} className="mt-4 w-full rounded-lg border border-zinc-800 py-2 text-xs font-medium text-zinc-400 hover:border-emerald-400/30 hover:text-emerald-300">Import connection</button></article>)}</div></section>
